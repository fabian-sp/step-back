"""
Adapted from https://github.com/epfml/llm-baselines/blob/main/src/data/shakespeare.py

* Character-level tokeization. The returned object contains the token ids, e.g. 39 --> F, 74 --> i.
* Tokenizer has 96 elements, but only the characters 0-91 appear in text. --> Vocab size = 92
...
def get_shakespeare(split, path):
    DATA_PATH = os.path.join(path, "shakespeare")
    ...
    which_path = train_path if split=="train" else test_path
    return np.memmap(which_path, dtype=np.uint16, mode="r")